s_box = [  # Implemented custom S-Box from paper: doi: 10.1016/j.protcy.2013.12.443
...
"""
Strict Avalanche Criterion (SAC) test for S-Box encryption: one bit difference in input should result in at least 50% of the bits changing in the output.
Bit Independence Criterion (BIC) test for S-Box encryption: the output bits should be independent of the input bits (i.e., the output bits should be uniformly distributed).
"""
...
def sac_bic_sbox_test(s_box):
    """
    Perform Strict Avalanche Criterion (SAC) and Bit Independence Criterion (BIC) tests for S-Box encryption.

    Args:
    - s_box: The S-Box to test.

    Returns:
    - True if the test passes, False otherwise.
    """
    changed_bits_count_sac = 0
    changed_bits_count_bic = 0

    for i in range(256):
        for j in range(8):
            # Flip one bit in the input
            input_modified = i ^ (1 << j)

            # Apply the S-Box to both original and modified input
            output = s_box[i >> 4][i & 0xF]
            output_modified = s_box[input_modified >> 4][input_modified & 0xF]

            # Count the number of bits changed in the output for SAC
            bit_difference = bin(output ^ output_modified).count('1')
            changed_bits_count_sac += bit_difference

            # For BIC, check if the output bits are independent
            if output & output_modified == 0:
                changed_bits_count_bic += 1

    # Check if both criteria are met
    sac_pass = changed_bits_count_sac >= 1024  # 256 inputs * 8 bits per input / 2
    bic_pass = changed_bits_count_bic >= 128  # 256 inputs / 2

    # Print both the SAC and BIC results
    perc1 = changed_bits_count_sac / 2048 * 100
    perc2 = changed_bits_count_bic / 256 * 100
    print(f"{Fore.GREEN}SAC: {changed_bits_count_sac} bits changed ({perc1:.2f}%) {Style.RESET_ALL}")
    print(f"{Fore.GREEN}BIC: {changed_bits_count_bic} bits are independent out of 256 ({perc2:.2f}%) {Style.RESET_ALL}")

    return sac_pass and bic_pass


class TestSACBICSBox(unittest.TestCase):
    def test_sac_bic_sbox(self):
        self.assertTrue(sac_bic_sbox_test(s_box))
